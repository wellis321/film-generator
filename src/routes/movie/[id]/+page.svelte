<script lang="ts">
	...
	let ourRating = $state(data.userMovie.ourRating ?? 5);
	let ourReview = $state(data.userMovie.ourReview ?? '');
	...
	<div class="min-w-0 flex-1">
		...
		<p class="mt-1 text-neutral-500">
			...
			{#if data.movie.certification}
				<span
					class="ml-1 inline-block rounded border border-neutral-700 px-1.5 py-0.5 text-xs font-semibold text-neutral-400"
					>{data.movie.certification}</span
				>
			{/if}
		</p>

		{#if data.userMovie.watched}
			<span
		...
		{#if user}
			...
			>
				...
				<div class="flex items-center gap-3">
					...
					>
						{data.userMovie.watched ? 'Update review' : 'Mark watched & save'}
					</button>
		...
		{:else}
			<div class="mt-3 rounded-2xl border border-neutral-800 bg-neutral-900 p-5 text-center">
				<p class="text-neutral-400">
					<a href="/login" class="text-amber-400 hover:underline">Log in</a> or
					<a href="/signup" class="text-amber-400 hover:underline">sign up</a> to rate and review this
					one.
				</p>
			</div>
		{/if}
